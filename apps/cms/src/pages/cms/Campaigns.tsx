...
export default function Campaigns() {
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      targetAudience: form.targetAudience.trim() || 'All subscribers',
      budget: form.budget,
    };
  ...
  const handleDelete = async (campaign: Campaign) => {
    try {
      ...
      if (detail?.id === campaign.id) setDetail(null);
    } catch (error) {
      toast.error(toApiError(error).message);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className={`px-3 py-2 text-xs border transition-smooth ${view === 'table' ? 'bg-foreground text-background border-foreground' : 'border-border hover:border-foreground'}`}
          >
            Table
          </button>
          <button
            onClick={() => setView('cards')}
            className={`px-3 py-2 text-xs border transition-smooth ${view === 'cards' ? 'bg-foreground text-background border-foreground' : 'border-border hover:border-foreground'}`}
          >
            Cards
          </button>
        </div>
                ...
                {paginated.map((c) => (
                  ...
                  >
                    <td className="px-5 py-3">
                      ...
                      {c.discountCode && (
                        <p className="text-xs text-muted-foreground font-mono">{c.discountCode}</p>
                      )}
                    </td>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {paginated.map((c) => (
              <div
                key={c.id}
                className="bg-background border border-border rounded p-5 transition-smooth hover:shadow-soft"
              >
                ...
                <div className="grid grid-cols-2 gap-3 pt-3 border-t border-border">
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    <MousePointer size={12} /> {c.clicks.toLocaleString()} clicks
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    <Users size={12} /> {c.conversions} converts
                  </div>
                  <div className="flex items-center gap-1.5 text-xs font-medium">
                    <DollarSign size={12} /> ${c.revenue.toLocaleString()}
                  </div>
                </div>
                <div className="flex gap-2 mt-4 pt-3 border-t border-border">
                  <button
                    onClick={() => setDetail(c)}
                    className="flex-1 py-2 border border-border text-xs font-medium letter-wide uppercase transition-smooth hover:border-foreground text-center"
                  >
                    View
                  </button>
                  {canEditCampaigns && (
                    <button
                      onClick={() => openEdit(c)}
                      className="flex-1 py-2 bg-foreground text-background text-xs font-medium letter-wide uppercase transition-smooth hover:opacity-90 text-center"
                    >
                      Edit
                    </button>
                  )}
                  {canDeleteCampaigns && (
                    <button
                      onClick={() => setDeleteConfirm(c)}
                      className="px-3 py-2 border border-border text-destructive text-xs transition-smooth hover:border-destructive"
                      title="Delete"
                    >
                      <Trash2 size={14} />
                    </button>
      ...
      {detail && (
        ...
        >
          <div
            className="bg-background w-full max-w-lg max-h-[85vh] overflow-y-auto m-4 shadow-elevated"
          ...
          >
            ...
            <div className="px-6 py-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs font-body font-semibold letter-wide uppercase text-muted-foreground mb-1">
                    Start
                  </p>
                  <p className="text-sm">{new Date(detail.startDate).toLocaleDateString()}</p>
                </div>
                <div>
                  <p className="text-xs font-body font-semibold letter-wide uppercase text-muted-foreground mb-1">
                    End
                  </p>
                  <p className="text-sm">{new Date(detail.endDate).toLocaleDateString()}</p>
                </div>
                <div>
                  <p className="text-xs font-body font-semibold letter-wide uppercase text-muted-foreground mb-1">
                    Budget
                  </p>
                  <p className="text-sm font-medium">${detail.budget.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-xs font-body font-semibold letter-wide uppercase text-muted-foreground mb-1">
                    Revenue
                  </p>
                  <p className="text-sm font-medium">${detail.revenue.toLocaleString()}</p>
                </div>
              </div>
              {detail.discountCode && (
                <div>
                  ...
                  <p className="text-sm font-mono">{detail.discountCode}</p>
                </div>
              )}
              <div className="border-t border-border pt-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="text-center p-3 bg-secondary rounded">
                    <p className="text-lg font-heading font-semibold">
                      {detail.clicks.toLocaleString()}
                    </p>
                    <p className="text-[10px] text-muted-foreground">Clicks</p>
                  </div>
                  <div className="text-center p-3 bg-secondary rounded">
                    <p className="text-lg font-heading font-semibold">{detail.conversions}</p>
      ...
      {deleteConfirm && (
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="font-heading text-lg mb-2">Delete Campaign</h2>
          ...
          >
            ...
            <div className="px-6 py-6 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, targetAudience: e.target.value.slice(0, 100) })
                  }
                  className="w-full px-3 py-2.5 border border-border text-sm bg-background focus:outline-none focus:border-foreground transition-smooth"
                  placeholder="e.g. All subscribers, VIP members"
                />
              </div>
              <div>
                <label className="block text-xs font-body font-medium letter-wide uppercase text-muted-foreground mb-1.5">
            ...
            <div className="px-6 py-4 border-t border-border flex justify-end gap-3">
              ...
              <button
                ...
                disabled={!form.name.trim() || !form.startDate || !form.endDate || isSaving}
