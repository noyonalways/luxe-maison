...
export function validatePromoCode(code: string, orderTotal: number): { valid: boolean; promo?: PromoCode; error?: string } {
  const promo = promoCodes.find(p => p.code === code.toUpperCase().trim());
  if (!promo) return { valid: false, error: 'Invalid promo code' };
  if (orderTotal < promo.minOrder) return { valid: false, error: `Minimum order of $${promo.minOrder} required` };
  return { valid: true, promo };
}

export function calculateDiscount(promo: PromoCode, orderTotal: number): number {
  if (promo.type === 'percentage') return Math.round(orderTotal * (promo.value / 100) * 100) / 100;
  return promo.value;
}
